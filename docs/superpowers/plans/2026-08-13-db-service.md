...
- **已知项**：① SafeStorageKeyProvider 运行时验证推迟 6/6（3/6 用 StaticKeyProvider 测）；② 默认 cipher sqleet（非 SQLCipher 格式），POC 自建自读够用；③ `require('electron')` 动态取 safeStorage，CJS 下可用；④ register.ts `wrap` 序列化 DbError 为 SerializedDbError 普通对象，但 preload 未反序列化——renderer 拿到普通对象（非 DbError 实例）。3/6 不跑 Electron 冲烟，DbView 错误路径不被执行（仅组件测试，mock 了 window.api），故不影响；6/6 接入时若需 renderer 拿到 DbError 实例再补 preload 反序列化。
